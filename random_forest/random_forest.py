...
class RandomForestAnalyzer:
    def __init__(self):
        ...
        self.hotEncoder = OneHotEncoder(sparse_output=False, drop='first', handle_unknown='ignore')
        self.Xhot = self.hotEncoder.fit_transform(self.X)

        self.Xhot = pd.DataFrame(
            self.Xhot,
            columns=self.hotEncoder.get_feature_names_out()
        )
        ...
        ## Split data
        self.x_train, self.x_test, self.y_train, self.y_test  = train_test_split(self.Xhot, self.y, test_size=0.25, random_state=42)
    ...
    def compute_importance_feature(self):
        ...
        feature_names = self.Xhot.columns
    ...
    def test(self):
        ...
        predict = self.model_rf.predict(self.x_test)
